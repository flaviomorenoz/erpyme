...
class Site extends CI_Model
{
    ...
    public function getProductByID($id, $store_id = NULL) {
        ...
        // product_id, quantity, price, price_rappi, price_pedidosya
        $jpsq = "( SELECT * from {$this->db->dbprefix('product_store_qty')} WHERE store_id = ".($store_id ? $store_id : "''")." ) AS PSQ";
    ...
    public function getAllCustomers() {
        ...
        $q = $this->db->select("*")->order_by("name","asc")->get('customers');
        if ($q->num_rows() > 0) {
    ...
    public function getAllUsers() {
        $this->db->select("{$this->db->dbprefix('users')}.id as id, username, first_name, last_name, {$this->db->dbprefix('users')}.email, company, {$this->db->dbprefix('groups')}.description as group, active, {$this->db->dbprefix('stores')}.name as store")
