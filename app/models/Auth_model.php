...
class Auth_model extends CI_Model {
    ...
    public function login($identity, $password, $remember = FALSE) {
        ...
        if ($query->num_rows() === 1) {
            ...
            if ($password === TRUE) {
                ...
                $cadena = $this->platillos_sin_receta();
                if(strlen($cadena)>0){
                    $cadi = "Recordarles que hay productos ({$cadena}) sin colocarles sus Recetas.";
                    $this->set_message($cadi);
                }else{
    ...
    public function platillos_sin_receta(){
        $categ_con_receta = "2,4,5,6";
        /* 
        2 Choripapas
        4 Salchipapas
        5 Sandwiches
        6 Tacos y Enchiladas*/

        /* Productos sin receta */
        $cSql = "SELECT a.code, a.name, a.category_id, b.nombreReceta, b.id_insumo 
            from `tec_products` a
            left join tec_recetas b on a.id = b.product_id 
            where a.`category_id` in ({$categ_con_receta}) and b.nombreReceta is null
            order by a.name";
