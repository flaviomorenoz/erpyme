function loadItems(){
	get("spoitems")&&(total=0,$("#poTable tbody").empty(),
		...
		$.each(spoitems,function(){
			...
			tr_html+='<td class="text-right"><span class="text-right ssubtotal" id="subtotal_'+d+'">'+formatMoney(parseFloat(s)*parseFloat(i))+"</span></td>"
			tr_html+='<td class="text-center"><i class="fa fa-trash-o tip pointer spodel" id="'+d+'" title="Remove"></i></td>'
			r.html(tr_html)
			r.prependTo("#poTable")
			total+=parseFloat(s)*parseFloat(i)
		})
...
function add_order_item(t){
	...
	$(document).ready(function(){
		...
		$(document).on("click",".spodel",function(){
			...
			if(delete spoitems[e],t.remove(),!spoitems.hasOwnProperty(e))
				return localStorage.setItem("spoitems",JSON.stringify(spoitems))
			void loadItems()
		})
