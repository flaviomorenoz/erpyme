<?php (defined('BASEPATH')) OR exit('No direct script access allowed'); 
    function casilli($store, $tipui, $query){
        $price = "";
        foreach($query->result() as $r){
            $price = $r->price;
        }
        $cad = "<div class=\"form-group st\">";
        $cad .= "<input type=\"text\" name=\"cas_{$store}_{$tipui}\" id=\"cas_{$store}_{$tipui}\" class=\"form-control\" value=\"$price\">";
                    ...
                    <div class="col-lg-12">
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-6">
                                ...
                                <!-- AQUI EMPIEZA LOS PRECIOS POR TIENDA -->
                                ...
                                ?>
                                                <div class="col-sm-3" style="margin-right:10px;">
                                                    ...
                                                    foreach($tipuis->result() as $tipui){
                                                        //echo "select * from tec_product_store_entes where store_id = {$store->id} and tipo_id = {$tipui->id} and product_id = {$product->id} <br>";

                                                        $cSql = "select * from tec_product_store_entes where store_id = ? and tipo_id = ? and product_id = ?";
                                                        $resultX = $this->db->query($cSql, array($store->id, $tipui->id, $product->id));
                                                        echo "<label>".$tipui->descrip."</label>" . casilli($store->id, $tipui->id, $resultX);
                                                    }
                                                    //die();
                                                    ?>
                                                </div>
