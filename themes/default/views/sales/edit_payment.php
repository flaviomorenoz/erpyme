                                ...
                                <div class="form-group">
                                    ...
                                    <?php
                                        if($this->Admin){
                                            $fp        = $payment->paid_by;
                                            $ar         = array();
                                            //$result     = $this->db->query("select forma_pago, descrip from tec_forma_pagos where activo='1'")->result_array();
                                            $cSql = "select forma_pago, descrip from tec_forma_pagos where activo='1'";
                                            $result = $this->db->query($cSql)->result_array();
                                            //$ar_p[""] = "--- Seleccione Tipo ---";

                                            $ar         = $this->fm->conver_dropdown($result, "forma_pago", "descrip", array(''=>'--- Seleccione Tipo ---'));
                                            echo form_dropdown('paid_by', $ar, $fp,'class="form-control" id="dni" required="required" onclick=""');
                                        }
                                    ?>
                                </div>
                            </div>
